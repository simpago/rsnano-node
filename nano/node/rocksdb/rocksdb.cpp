	...
	cf_name_table_map{ create_cf_name_table_map () }
{
	...
	auto single_column_family = [this] (std::string const cf_name) {
		std::vector<::rocksdb::ColumnFamilyDescriptor> minimum_cf_set{
			{ ::rocksdb::kDefaultColumnFamilyName, ::rocksdb::ColumnFamilyOptions{} },
			{ cf_name, this->get_cf_options (cf_name) }
		};
	...
	open (is_fresh_db, path_a, true, single_column_family ("meta"));
	...
	if (is_fully_upgraded || is_fresh_db)
	{
		if (is_fresh_db)
		{
			// There is no database yet, the access needs to be in write mode for it to be created.
			if (open_read_only_a)
			{
				error = true;
				return;
		...
		open (error, path_a, open_read_only_a, create_column_families ());
	}
	else
	{
		// It is not possible to run upgrades in read-only mode.
		if (open_read_only_a)
		{
			error = true;
			return;
		...
		// If the database is not upgraded, it may not be compatible with the current column family set.
		std::vector<::rocksdb::ColumnFamilyDescriptor> column_families;
		{
			std::vector<std::string> current_cf_names;
			auto s = ::rocksdb::DB::ListColumnFamilies (get_db_options (), path_a.string (), &current_cf_names);
			if (!s.ok ())
			{
				error = true;
				return;
		...
		open (error, path_a, open_read_only_a, column_families);
		if (!error)
		{
...
void nano::rocksdb::store::open (bool & error_a, boost::filesystem::path const & path_a, bool open_read_only_a, std::vector<::rocksdb::ColumnFamilyDescriptor> column_families)
{
