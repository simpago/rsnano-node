...
use rsnano_node::{
    config::{get_node_toml_config_path, NetworkConstants, TomlDaemonConfig},
    NetworkParams,
};
use toml::from_str;
...
pub(crate) struct CurrentConfigArgs {
    ...
    pub(crate) fn update_config(&self) -> Result<()> {
        ...
        if self.node {
            ...
            if node_toml_config_path.exists() {
                ...
                let current_toml_daemon_config: TomlDaemonConfig = from_str(&toml_str)?;
                let default_toml_daemon_config =
                    TomlDaemonConfig::default(&network_params, get_cpu_count())?;
                let merged_config =
