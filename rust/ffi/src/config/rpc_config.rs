...
use rsnano_node::config::{NetworkConstants, RpcConfigToml, RpcLoggingConfig, RpcProcessConfig};
...
pub unsafe extern "C" fn rsn_rpc_config_create(
    ...
    let cfg = RpcConfigToml::new(&network_constants, get_cpu_count());
    let dto = &mut (*dto);
...
pub unsafe extern "C" fn rsn_rpc_config_create2(
    ...
    let cfg = RpcConfigToml::new2(&network_constants, get_cpu_count(), port, enable_control);
    let dto = &mut (*dto);
    fill_rpc_config_dto(dto, &cfg);
    0
}

fn fill_rpc_config_dto(dto: &mut RpcConfigDto, cfg: &RpcConfigToml) {
...
pub extern "C" fn rsn_rpc_config_serialize_toml(dto: &RpcConfigDto, toml: *mut c_void) -> i32 {
    let cfg = match RpcConfigToml::try_from(dto) {
        Ok(c) => c,
        Err(_) => return -1,
    };
...
impl TryFrom<&RpcConfigDto> for RpcConfigToml {
    type Error = anyhow::Error;

    fn try_from(dto: &RpcConfigDto) -> Result<Self, Self::Error> {
        let cfg = RpcConfigToml {
